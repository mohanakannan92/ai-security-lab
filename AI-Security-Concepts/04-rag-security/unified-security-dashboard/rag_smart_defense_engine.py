...
def run_rag(query):
    ...
    # -------------------------------
    # Semantic + Confidence Layer
    # -------------------------------
    decision = confidence_guard(query)

    stage_data["semantic_flag"] = decision["risk"] != "low"

    # BLOCK
    if decision["action"] == "block":
        log_attack(query, decision, stage_data)
    ...
    stage_data["retrieved_docs"] = len(retrieved_docs)

    safe_docs = sanitize_context(retrieved_docs)

    stage_data["sanitized_docs"] = len(safe_docs)

    stage_data["malicious_detected"] = len(safe_docs) < len(retrieved_docs)
